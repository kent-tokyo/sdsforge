...
`other_names`・`measured_properties`（およびその中の各物性項目）・
`evidence` のような空でよい任意項目は、入力ファイルから省略できます —
キーの省略は空の値を指定した場合と同じ意味になります。製品名・サプラ
イヤー・成分・濃度・単位・参照先エビデンスのメタデータは引き続き必須
であり、未知のキーは拒否されるため、入力ファイルの誤字（例:
`concentation:`）は黙って無視されずパースエラーになります。正確な規則
は [`examples/generate/README.md`](examples/generate/README.md) を参照
してください。
