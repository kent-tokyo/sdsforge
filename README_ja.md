...
| [`sds-converter`](./sds_converter/) | CLIバイナリ + GUIアプリケーション — `to-json`・`to-docx`・`to-html`・`to-pdf`・`validate`・`extract-text` サブコマンド |
...
- **GUIアプリケーション**（eframe/egui）: 引数なしで起動するとGUIウィンドウを表示。変換・文書生成・検証・テキスト抽出・設定の5タブ構成。ドラッグ&ドロップ対応・設定ファイル永続化。
