...
> 本页面详细说明 sdsforge 生成的 SDS JSON 文件的自动质量验证机制及各项检查规则。
...
| **r27** 活性信号词＋H码存在但 `Pictogram` 列表完全为空 | MED | PDF 中象形图仅以图片形式嵌入，无法提取为 GHS01–GHS09 文本码。随机30件测试中约60%文件出现此问题。对于前 GHS MSDS 文件属数据源限制 |
...
| **r27** 混合物组分浓度有单位但无数值 | MED | `NumericRangeWithUnitAndQualifier.Unit`（如 `"%"`）已设置，但 `ExactValue`/`LowerValue`/`UpperValue` 全部缺失 — LLM 识别了单位却遗漏了数值 |
...
| **r27** | **新规则**：S2-HAZARD-NO-PICTOGRAM（MED）活性信号词＋H码存在但Pictogram列表为空；S3-CONC-UNIT-NO-VALUE（MED）混合物组分浓度有单位但无数值。**误报修复**：将`危險`（zh-tw繁体"危险"）和`Not applicable`（英文非危险品）加入有效信号词；S14 UN编号（`聯合國編號(UN No.)：XXXX`等）、包裝類別/包裝等級、聯合國運輸名稱等繁体/简体中文格式扩展 |
